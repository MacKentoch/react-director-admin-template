...
import { fetchMockTeamMatesData } from '../../services';
import { getTeamMatesData } from '../../services/API';
...
function fetchTeamMatesData() {
  return dispatch => {
    ...
    if (appConfig.DEV_MODE) {
      fetchMockTeamMatesData().then(data =>
        dispatch(receivedTeamMatesData(data)),
      );
    } else {
      getTeamMatesData()
        .then(data => dispatch(receivedTeamMatesData(data)))
        .catch(error => dispatch(errorTeamMatesData(error)));
    }
  };
}
